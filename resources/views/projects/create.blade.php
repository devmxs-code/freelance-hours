<x-layouts.app>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-300 p-8">
        <h1 class="text-3xl font-bold text-black mb-6">Criar Novo Projeto</h1>
        ...
        <form action="{{ route('projects.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          rows="8"
                          required
                          class="w-full px-4 py-2 border border-gray-400 rounded focus:ring-2 focus:ring-black focus:border-black bg-white text-black @error('description') border-gray-600 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-gray-700">{{ $message }}</p>
                @enderror
            </div>
